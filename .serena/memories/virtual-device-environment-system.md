...
- **cron-job.org**: Each environment gets its own CRON job running every 5 minutes
...
| Speed | Readings per Trigger | Completion Time (440 rows) |
|-------|---------------------|---------------------------|
| 1X    | 1                   | ~96 days                  |
| 2X    | 2                   | ~48 days                  |
| 5X    | 5                   | ~19 days                  |
...
All environments run every 5 minutes via cron-job.org. The speed setting controls how many CSV rows are processed per trigger, not the schedule frequency.
...
5. New CRON job created on cron-job.org
